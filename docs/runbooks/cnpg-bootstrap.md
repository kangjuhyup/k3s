...
- 접속 주소 `shared-postgres-rw.databases.svc.cluster.local:5432`; Doppler에 `DB_HOST`, `DB_PORT`, `DB_SSLMODE=verify-full`을 추가했다. 외부 노출은 없다.
...
- 입력: `gitops/clusters/oci-a1/cnpg.json`, values: `gitops/platform/cnpg/base.values.json`.
- 생성: `scripts/argocd_gitops.py --repo-root . --write`; 생성물 직접 편집 금지.
...
직접 Helm 설치·kubectl apply 대신 Git 변경을 Argo CD가 반영한다. prune=false이며 설정 비활성화로 기존 Operator를 조용히 방치하는 변경은 생성기가 거부한다. Operator 제거·CRD 삭제는 DB 리소스에 영향을 줄 수 있어 별도 검토가 필요하다. Git revert는 DB 데이터 복원이 아니다.
