...
- [base.values.yaml](base.values.yaml): controller/webhook/cainjector 각 1 replica, ARM64/Linux, image digest, 요청량 합계 200m CPU / 320Mi RAM.
- [versions.json](versions.json): chart `v1.21.1`, SHA256, Helm `v4.2.4`, Kubernetes 지원 범위 `1.33~1.36`.
- [Application](../../clusters/oci-a1/root/cert-manager.yaml)과 [매니페스트](../../clusters/oci-a1/cert-manager/): 직접 관리하는 설치 원본.
- [선언 검사](../../../scripts/gitops_validate.py): 파일을 변경하지 않는 오프라인 검사.
...
auto prune=false, Application finalizer 없음, namespace Prune/Delete 보호, CRD의 Helm keep, Certificate Secret ownerRef 자동 삭제 비활성을 유지한다. 파일을 삭제하는 것은 제거 절차가 아니며 prune=false로 기존 리소스가 남을 수 있다. 실제 폐기는 CRD·인증서·연쇄 삭제 영향을 별도 검토한다.
